...
\SSsect[!] \( x \in \underset{F}{Lim}(f) ~\Rightarrow~ \Gamma_x \) есть фильтр \( ~\Rightarrow~ \exists F' \) --- более тонкий фильтр, чем \( F \) , такой, что \( x = \lim\limits_{F'} f ~\Rightarrow~ \exists F' \) --- более тонкий фильтр, чем \( F \) , такой, что \( x \in \underset{F'}{Lim}(f) ~\Rightarrow~ x \in \underset{F}{Lim}(f) \) .
